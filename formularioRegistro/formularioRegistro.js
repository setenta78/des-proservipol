...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fechaInicio = new Date(2026, 2, 13, 0, 0, 0);   // PRUEBAS: 13/03/2026
    const fechaLimite = new Date(2026, 2, 27, 23, 59, 59); // Producción: 27/03/2026

    if (fecha >= fechaInicio && fecha <= fechaLimite) {
        btnBuscar.disabled = false;
        btnRegistrar.disabled = false;
        textCodFuncionarioBusqueda.disabled = false;
        document.getElementById('labelTextRegistro').textContent = "Código de Funcionario";
...
async function buscarFuncionario(){
    ...
    if(fecha < fechaInicio || fecha > fechaLimite){
        alert("Inscripción cerrada. El período de inscripción es del 16 al 27 de marzo de 2026.");
        return;
    }
...
async function matricular(){
    ...
    const fecha      = new Date();
    const fechaLimite = new Date(2026, 2, 27, 23, 59, 59); // 27/03/2026 23:59
