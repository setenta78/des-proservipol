 ...
 * Lógica de Autenticación con Autentificatic API
...
var CONFIG = {
    ...
    REDIRECT_URL: 'http://des-proservipol.carabineros.cl/unidades.php?login=true'
};
...
function guardarSesion(loginData, userData) {
    return new Promise(function(resolve, reject) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                console.log('Respuesta de save_token.php:', xhr.responseText);
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (response.success) {
                            ...
                            console.error('❌ Error al guardar sesión:', response.message);
                            reject({ message: response.message || ERROR_MESSAGES.SESSION_ERROR });
                        }
                    } catch (e) {
        ...
        console.log('Enviando datos a save_token.php...');
        xhr.send(body);
    });
}
